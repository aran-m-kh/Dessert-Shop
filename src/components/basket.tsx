...
const Basket = () => {
    ...
    if (cartItems.length === 0) {
        ...
        return <div className="h-fit mob:w-3/4 mob:m-auto mob:my-2 tab:my-2 tab:w-3/4 tab:m-auto w-1/4 mt-11 mr-11 flex flex-col items-center bg-slate-100 bg-clip-padding rounded-md backdrop-filter backdrop-blur-sm bg-opacity-100 backdrop-saturate-50 backdrop-contrast-100">
            
            <h1 className="text-2xl mx-auto my-3 items-center px-10 py-2 bg-slate-300 border text-orange-700 border-orange-500/100 rounded-3xl">Your Cart {cartItems.length}</h1>
        
            {cartItems.map((item, index) =>
                data.map((data) =>{
                    if (item.id === data.id) {
                        return <div className="bg-slate-200 m-2 p-3 w-11/12 border border-orange-500/100 rounded-3xl transition-all duration-300 ease-in-out cursor-pointer select-none hover:border-emerald-800 hover:bg-teal-200 focus:shadow-xs focus:no-underline" key={index}>
            ...
            <div className="flex flex-row tab:w-3/4 tab:justify-center my-4 items-center bg-slate-200 rounded-md p-2">
                ...
                <h3 className="m-1">this is a <span className="font-bold">carbon-neutral</span> delivery</h3>
            </div>
